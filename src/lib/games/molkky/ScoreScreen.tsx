...
export function MolkkyScoreScreen({
  matchId,
  gameCode,
  participants,
  initialRounds,
  ...
  const winners = isCompleted ? determineWinners(rounds, participantIds) : [];
  const winnerPoints = winners.length > 0 ? totals[winners[0]] ?? 0 : 0;
  const reached50 = !isCompleted ? Array.from(finishedPlayers(rounds, participantIds)) : [];
  const eliminated = eliminatedPlayers(rounds, participantIds);
  const approachingTarget = !isCompleted ? playersApproachingTarget(totals) : [];
  const outRounds = outAtRound(rounds, participantIds);
  ...
  function finishMatch() {
    const finalWinners = determineWinners(rounds, participantIds);
    startTransition(async () => {
      await Promise.all(
      ...
      {reached50.length > 0 && (
        <div className="card flex items-center gap-2" style={{ borderColor: "#163D2E" }}>
          <span className="font-quicksand text-sm font-semibold text-onjoo-green-900">
            {participants
              .filter((p) => reached50.includes(p.id))
              .map((p) => p.name)
              .join(", ")}{" "}
            {reached50.length > 1 ? "ont atteint" : "a atteint"} {TARGET_SCORE} pts — la partie
            peut se terminer.
          </span>
        </div>
      )}
            ...
            {orderedParticipants.map((participant, index) => (
              <div key={participant.id} className="flex flex-col items-center gap-1 px-1 py-2.5">
                ...
                {eliminated.has(participant.id) && (
                  <span className="font-quicksand text-[10px] font-bold text-onjoo-red-500">
                    Éliminé
                  </span>
                )}
              </div>
...
function RoundRow({
  ...
  participants: Participant[];
  rounds: Round[];
  outRounds: Record<string, number | null>;
  ...
  return (
    <>
      ...
      {participants.map((participant) => {
        ...
        const outRound = outRounds[participant.id];
        // Sorti (éliminé ou déjà à 50) AVANT ce tour : plus rien à saisir.
        const isOut = outRound !== null && outRound !== undefined && roundIndex > outRound;
        ...
        return (
          <div key={participant.id} className="flex items-center justify-center bg-white px-1 py-2">
            {isOut ? (
              <span className="flex h-11 w-14 items-center justify-center font-quicksand text-lg text-[#ddd]">
                —
              </span>
            ) : (
              <ScoreCell
...
function ScoreCell({
  ...
  function commit() {
    ...
    const clamped = Math.max(0, Math.min(MAX_THROW, Number(value) || 0));
    onSave(clamped, {});
  }
