...
export function Le5ScoreScreen({
  matchId,
  gameCode,
  participants,
  initialRounds,
        ...
        <div className="card flex items-center gap-2" style={{ borderColor: "#d64545" }}>
          <span className="font-quicksand text-sm font-semibold" style={{ color: "#d64545" }}>
            {participants
              ...
              .map((p) => p.name)
              .join(", ")}{" "}
            {overTarget.length > 1 ? "ont dépassé" : "a dépassé"} {TARGET_SCORE} pts — la partie peut
            se terminer (rappel : le score le plus bas gagne).
          </span>
        </div>
      )}
...
function ScoreCell({
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <input
        // Pas d'inputMode="numeric" : au 5 les scores peuvent être négatifs
        // (rois = -1, score du gagnant plafonné à 0 ou moins), il faut
        // garder le clavier complet avec "-".
        type="number"
        value={value}
        disabled={disabled}
